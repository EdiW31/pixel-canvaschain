...
const AdminPage = () => {
  ...
  const handleEndEpoch = async () => {
    ...
    try {
      ...
      // Painter NFT URI points at the AI-generated image. The server starts
      // generating it asynchronously in the POST /snapshots/epoch handler
      // above; while it's in flight, GET /painter.png falls through to
      // canvas.png so the NFT image is never broken. The on-chain URI never
      // changes — only the file behind it appears once the AI job finishes.
      // Auction NFT stays as the raw 20×20 zone snapshot (user requirement).
      let painterUri = `${SERVER_URL}/snapshots/epoch/${epoch}/painter.png`;
          ...
          <Card title="End Epoch & Distribute + Mint NFTs" description="Distributes PIXEL & EGLD to charities, mints 2 NFTs, resets canvas." accent="#E53E3E" badge={<StepBadge n={2} done={false} />}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              ...
              <p style={{ fontSize: 11, color: 'rgb(var(--text-muted))' }}>
                Canvas resets ~30 s after confirmation. The painter NFT image is generated by AI in the background (~20–30 s) and replaces the canvas snapshot in the gallery automatically.
              </p>
            </div>
          </Card>
        </div>
